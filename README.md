...
3. Open `config.cfg` and adapt the paths. Always use Unix-style forward slashes (`/`) as separators.
   ...
   :warning: **Note for macOS users** If your GNU `getopt` installation path deviates from its typical installation paths (see [3. Confirming installation](#3-confirming-installation-1), replace the default value of the `GETOPT_PATH` variable with the path that you noted down; **make sure it ends with a `/`**.  
